...
describe "Ext4::Dir class methods" do
  before(:all) do
    reset_context
  ...
  describe ".entries" do
    it "should raise RuntimeError when given a nonexistent directory" do
      expect do
        ...
        RuntimeError, /Directory '\/nonexistent_directory' not found/
      )
    end

    it "should, given full path, return the same file names as the real Dir.entries" do
      expect(VirtFS::VDir.entries(@root)).to match_array(@ext.root_dir + [".", ".."])
    end

    it "should, given relative path, return the same file names as the real Dir.entries" do
      expect(VirtFS::VDir.entries(".")).to match_array(@ext.root_dir + [".", ".."])
    end
  end
  ...
  describe ".foreach" do
    ...
    it "should, given full path, return the same file names as the real Dir.foreach" do
      expect(VirtFS::VDir.foreach(@root).to_a).to match_array(@ext.root_dir + ['.', '..'])
    end

    it "should, given relative path, return the same file names as the real Dir.foreach" do
      expect(VirtFS::VDir.foreach(".").to_a).to match_array(@ext.root_dir + ['.', '..'])
    end
  end
  ...
  describe ".new" do
    it "should raise Errno::ENOENT when directory doesn't exist" do
      expect do
        ...
        RuntimeError, /Directory '\/not_a_dir' not found/
      )
    end
